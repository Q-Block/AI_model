...
# 모델 로드
loaded_model = joblib.load('eTc.sav')

# 모델이 정상적으로 로드되었는지 확인
print(type(loaded_model))
print(hasattr(loaded_model, 'predict'))


#모델 로드 (Pickle 사용)
#with open('eTc.sav', 'rb') as file:
#    loaded_model = pickle.load(file)
#    print("pickle model load 성공")

# 모델이 로드되었는지 확인
#print(type(loaded_model))

# 모델이 predict 함수를 가지고 있는지 확인
#print(hasattr(loaded_model, 'predict'))


# 예측 결과를 매핑하는 딕셔너리
pred_dict = {0: 'benign', 1: 'defacement', 2: 'phishing', 3: 'malware'}
...
# URL의 악성 여부를 AI 모델을 통해 판단하는 함수
def predict_malicious(url):
    # URL에서 수치형 특성 추출
    numerical_values = get_numerical_values(url)
    
    # 모델에 입력할 수 있도록 변환
    url_features = np.array(list(numerical_values.values())).reshape(1, -1)
    
    # 모델을 사용하여 예측
    prediction_int = loaded_model.predict(url_features)[0]
    
    # 예측 결과가 benign(정상)일 경우, '정상', 그 외는 '악성'으로 반환
    if prediction_int == 0:
        return '정상'
    else:
        return '악성'

# AI 서버로 URL, url_id, qr_status가 들어오면 처리하는 엔드포인트
@app.route('/check-url', methods=['POST'])
def check_url():
    data = request.json
    url = data.get('url')
    url_id = data.get('url_id')
    qr_status = data.get('qr_status')
    
    # AI 모델을 통해 악성 여부 판단
    malicious_status = predict_malicious(url)
